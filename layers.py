...
class BottleneckResidualBlock(nn.Module):
    ...
    def __init__(self, first_channel, last_channel, factor, stride):
        super().__init__()
        self.stride = stride
        self.conv_1 = nn.Conv2d(in_channels=first_channel, out_channels=int(first_channel*factor), kernel_size=1, stride=2)

        # channel to be used in the block
        c = self.conv_1.out_channels
        self.bn_1 = nn.BatchNorm2d(c)

        self.conv_2_dw = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, stride=stride, groups=c, padding='same' if stride == 1 else 'valid')
...
class MobileNetV2(nn.Module):
    def __init__(self, class_num):
        ...
        self.layer_1_conv = nn.Conv2d(in_channels=3, out_channels=32, stride=2, kernel_size=3)
        ...
        self.layer_3_bottleneck = nn.ModuleList(tmp_lst)
        ...
        self.layer_4_bottleneck = nn.ModuleList(tmp_lst)
        ...
        self.layer_5_bottleneck = nn.ModuleList(tmp_lst)
        ...
        self.layer_6_bottleneck = nn.ModuleList(tmp_lst)

        tmp_lst = []
        for i in range(3):
            if i == 0:
                tmp_lst.append(BottleneckResidualBlock(first_channel=self.layer_5_bottleneck[-1].last_channel, last_channel=160, factor=6, stride=2))
            else:
                tmp_lst.append(BottleneckResidualBlock(first_channel=160, last_channel=160, factor=6, stride=1))
        self.layer_7_bottleneck = nn.ModuleList(tmp_lst)
